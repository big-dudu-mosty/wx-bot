...
3. Repair candidate splitting, then repeat the multi-message parser check in the verified `共享中` state. The 2026-07-30 `C1`–`C6` run produced one candidate for each visible right-side text bubble, but their sender remains `unknown` and the same screen's mini-program card produced UI-text candidates. A later long-text test combined each fully visible `dd` recruitment bubble into one candidate. A bubble whose nickname is clipped outside the viewport remains `unknown`, which is correct. Gate 3 remains open: classify card/media UI as unsupported rather than text messages, and deduplicate overlapping observations without dropping legitimate repeated text.
